...
public class MainActionBuyPermitCardTest {
    ...
    public void testBuy() throws Exception {
        this.user.setCoinPathPosition(20);
        this.user.setMainActionCounter(2);
        ArrayList<PoliticCard> politicCardArrayList = new ArrayList<>();
        ArrayList<Councilor> councilors = new ArrayList<Councilor>(this.game.getRegion(RegionName.MOUNTAIN).getCouncil().getCouncil());
        ...
        this.user.getPoliticCards().addAll(politicCardArrayList);
        RegionName region = RegionName.MOUNTAIN;
        PermitCard permitCard = this.game.getPermitDeck(RegionName.MOUNTAIN).getVisibleArray().get(0);
        MainActionBuyPermitCard mainActionBuyPermitCard = new MainActionBuyPermitCard(politicCardArrayList, region, permitCard);
        mainActionBuyPermitCard.doAction(this.game, this.user);

        assertFalse(this.game.getPermitDeck(RegionName.MOUNTAIN).getVisibleArray().contains(permitCard));

        assertTrue(this.user.getPermitCards().contains(permitCard));
